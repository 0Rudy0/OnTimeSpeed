...
{
    public class ColleagueSupportEntry : ISemiAutomaticEntry
    {
        public async Task<List<WorkItem>> GetAllRelatedTasks(User user)
        {
            string cacheKey = "colegueSupportTasks";
            var searchStrings = SearchStrings.Get().FirstOrDefault(s => s.Name == "podrskaKolegiSearchString").SearchStrings;
        ...
        {
            return PrepareData.GetTaskForDate(items, forDate, "podrskaKolegiTemplate");
        }
